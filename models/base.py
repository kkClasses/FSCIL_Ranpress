...
class BaseLearner(object):
    ...
    def _eval_cnn(self, loader):
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            inputs = inputs.to(self._device)
            with torch.no_grad():
                out  = self._network(inputs) #["logits","features"]
                #print("embed shape",out["features"].shape)
                outputs = out["logits"] #
            if self._cur_task == 8:
                feats = out["features"].cpu().numpy()  # shape: (batch_size, feature_dim)
                labels = targets.cpu().numpy().reshape(-1, 1)  # shape: (batch_size, 1)
                data = np.hstack((feats, labels))  # shape: (batch_size, feature_dim + 1)
                with open("mini_embed_768.csv", 'ab') as f:  # append mode
